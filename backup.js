...
(function () {
    ...
    function loadVandapQA(callback) {
        if (vandapQAReady) {
        ...
        if (vandapQALoading) {
            setTimeout(function () {
                loadVandapQA(callback);
            }, 250);
            return;
        }

        vandapQALoading = true;
        fetch(vandapQAUrl)
            ...
            .then(function (jsonData) {
                if (jsonData && typeof jsonData === 'object') {
                    storedQA = jsonData;
                    vandapQAReady = true;
                    showTempAlert('Đã tải xong danh sách câu hỏi Vấn Đáp', 'success');
                } else {
            ...
            .catch(function (err) {
                storedQA = {};
                showTempAlert('Không tải được file QA Vấn Đáp', 'error');
            })
            .finally(function () {
                vandapQALoading = false;
                if (typeof callback === 'function') callback();
            });
    }

    function getVandapOptions(root) {
        if (!root) {
            return [];
        }
        return Array.from(root.querySelectorAll('.options .option, .option'));
    }

    function getVandapAnswer(questionText) {
        if (!questionText) {
            return '';
        }
        questionText = questionText.trim();
        if (storedQA.hasOwnProperty(questionText)) {
            return storedQA[questionText];
        }

        for (var key in storedQA) {
            if (Object.prototype.hasOwnProperty.call(storedQA, key) && questionText.includes(key)) {
                return storedQA[key];
            }
        }
        return '';
    ...
    function clickElementBySelector(root, selector) {
        ...
        if (element) {
            element.click();
            // showTempAlert('Click' + selector + ' thành công!');
            return true;
        }
        return false;
    }
    ...
    function danh_te_le(root, closeIframe) {
        resetTimer('tele');
        if (!clickElementBySelector(root, '#te-le-button')) {
        ...
        setTimeout(function () {
            if (!clickElementBySelector(root, '.swal2-confirm.swal2-styled')) {
                showTempAlert('Đánh Tề Lễ không thành công');
                if (typeof closeIframe === 'function') closeIframe();
                return;
            }
            localStorage.setItem('tele_done', getToday());
            showTempAlert('Đánh Tề Lễ thành công', 'success');
            if (typeof closeIframe === 'function') closeIframe();
        }, 3000);
    }

    function danh_van_dap(root, closeIframe) {
        if (root.querySelector('#result')) {
        }
        if (!vandapQAReady) {
            if (!vandapQALoading) {
                loadVandapQA(function () {
                    danh_van_dap(root, closeIframe);
                });
            }
            return
        }
        ...
        function answerCurrentQuestion() {
            var questionEl = root.querySelector('#question');
            var options = getVandapOptions(root);
            var questionText = questionEl ? questionEl.textContent.trim() : '';
            var answerText = getVandapAnswer(questionText);

            if (answerText) {
                var target = options.find(function (btn) {
                    return btn.textContent.trim() === answerText;
                });
                if (target) {
                    target.click();
                    showTempAlert('Vấn đáp: chọn đáp án ' + answerText, 'success');
                    return;
                }
            }
            if (questionText) {
                showTempAlert('Không tìm thấy đáp án phù hợp cho câu hỏi: ' + questionText, 'error');
            }
        }

        setTimeout(function () {
            if (!clickElementBySelector(root, '#start-quiz-button')) {
                if (typeof closeIframe === 'function') closeIframe();
                return;
            }

            for (var i = 0; i < vandapQuestionCount; i++) {
                (function (index) {
                    setTimeout(function () {
                        answerCurrentQuestion();
                    }, vandapDelayMs * (index + 1));
                })(i);
            }

            setTimeout(function () {
                if (typeof closeIframe === 'function') closeIframe();
            }, vandapDelayMs * vandapQuestionCount + 3500);
        }, 3000);
    }
    ...
    function createControlPanel() {
        ...
        panel.innerHTML = `
            ...
            <button id="btn-vandap" class="panel-btn">Vấn Đáp</button>
        ...
        document.getElementById('btn-vandap').addEventListener('click', function () {
            if (this.disabled) return;
            danh_van_dap_in_iframe();
    ...
    function createAutoMenuButton() {
        ...
        if (!navItems) {
            // Fallback: tạo button trực tiếp
            createAutoMenuButtonFallback();
            return;
        }
    ...
    function danh_hoang_vuc_in_iframe() {
        getOrCreateIframe('hoangvuc-frame', hoangvuc, function (doc, closeIframe) {
    ...
    function danh_phuc_loi_in_iframe() {
        getOrCreateIframe('phucloi-frame', phucloi, function (doc, closeIframe) {
            var result = danh_phuc_loi(doc);
            setTimeout(function () {
                updateButtonStates();
            }, 100);
            return result;
    ...
    function danh_khoang_mach_in_iframe() {
        getOrCreateIframe('khoangmach-frame', khoangmach, function (doc, closeIframe) {
    ...
    function danh_diem_danh_in_iframe() {
        getOrCreateIframe('diemdanh-frame', diemdanh, function (doc, closeIframe) {
    ...
    function danh_thi_luyen_in_iframe() {
        getOrCreateIframe('thiluyen-frame', thiluyen, function (doc, closeIframe) {
    ...
    function danh_bi_canh_in_iframe() {
        getOrCreateIframe('bicanh-frame', bicanh, function (doc, closeIframe) {
    ...
    function danh_te_le_in_iframe() {
        getOrCreateIframe('tele-frame', tele, function (doc, closeIframe) {
    ...
    function danh_van_dap_in_iframe() {
        getOrCreateIframe('vandap-frame', vandap, function (doc, closeIframe) {
            var result = danh_van_dap(doc, closeIframe);
            setTimeout(function () {
                updateButtonStates();
            }, 100);
            return result;
    ...
    var taskIntervals = [
        ...
        { name: 'vandap', label: 'Vấn Đáp', time: 10* 60 * 1000, func: danh_van_dap_in_iframe }
    ];
    ...
    function runTaskTimers() {
        taskIntervals.forEach(function (task) {
            // Skip if task is done for today
            if (localStorage.getItem(task.name + '_done') === getToday()) {
                return;
            }
            ...
            if (elapsed >= task.time) {
                ...
                localStorage.setItem('task_timer_' + task.name, Date.now());
            }
        });
    }
