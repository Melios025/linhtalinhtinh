...
(function () {
    ...
    let storedQA = {
        ...
        'Bộ phim nào sau đây thuộc tiểu thuyết của tác giả Thiên Tằm Thổ Đậu':
            'Tất cả đáp án trên (ĐCT, VĐCK, ĐPTK)',
        ...
        'Công pháp nào sau đây là của Hàn Lập trong Phàm Nhân Tu Tiên ?':
            'Tất cả đáp án trên (MVQ, TXC, TNKQ)',
        ...
        'Nhân vật Bách Lý Đồ Minh xuất hiện trong phim hoạt hình nào dưới đây ?':
            'Trảm Thần Chi Phàm Trần Thần Vực',
        ...
        'Cố Hà trong Đấu Phá Thương Khung lúc xuất hiện ở Vân Lam Tông là luyện dược sư mấy phẩm?':
            'Lục Phẩm',
        ...
        'Trong Kiếm Lai, khi Man Châu Động Thiên đứng trước nguy cơ bị hủy diệt, là ai đã đứng ra bảo vệ người dân trong trấn?':
            'Tề Tĩnh Xuân',
        'Trong Na Tra: Ma Đồng Giáng Thế Na Tra được sinh ra từ gì?': 'Ma Hoàn',
        'Trong phim Đại Đạo Triều Thiên, Tỉnh Cửu đã cùng thư đồng đến đâu để tu luyện?':
            'Thanh Sơn Tông',
        ...
        'Trong phim Vạn Cổ Tối Cường Tông, Quân Thường Tiếu chiêu mộ ai lam đệ tử đầu tiên?':
            'Lục Thiên Thiên',
    ...
    function danh_van_dap(root) {
        console.log('danh_van_dap: Checking for #result in iframe');
        if (root.querySelector('#result')) {
            console.log('danh_van_dap: #result found, completed');
            alert('Đã hoàn thành vấn đáp, không cần làm gì nữa!');
            return true;
        }
        console.log('danh_van_dap: #result not found, not completed');
        return false;
    }


    function getOrCreateIframe(iframeId, url, onLoad) {
        console.log('getOrCreateIframe: Called with id=' + iframeId + ', url=' + url);
        ...
        function handleLoad() {
            ...
            var shouldClose = onLoad(doc);
            if (shouldClose && document.body.contains(iframe)) {
                iframe.remove();
            }
        }
    ...
    function danh_van_dap_in_iframe() {
        getOrCreateIframe('vandap-frame', vandap, function (doc) {
            return danh_van_dap(doc);
        });
    }
    ...
    if (window.location.hostname === 'hoathinh3d.co') {
        ...
        window.addEventListener('load', function () {
            console.log('Trang đã tải xong, bắt đầu vấn đáp...');
            danh_van_dap_in_iframe();
        });
        window.addEventListener('hashchange', function () {
           console.log('Hash changed, starting van dap...');
            danh_van_dap_in_iframe();
        });
    } else {
        console.log('Script not running: hostname is ' + window.location.hostname);
    }
})